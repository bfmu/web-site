...
function getReturnUrl(): string {
  ...
  if (
    !returnUrl ||
    returnUrl.includes('/admin/login') ||
    returnUrl.includes('/admin/register') ||
...
export async function loadOAuthProviders(): Promise<void> {
  ...
  try {
    const response = await fetch(
      `${getBackendApiUrl()}/settings/oauth/public/available`,
    )
...
/**
 * Init de la página /admin/login: redirige si ya hay sesión y procesa el
 * callback OAuth (?token=&refresh=) al volver de Google/GitHub.
 * Se llama desde Layout.astro en carga directa y tras Swup content:replace.
 */
export function initLoginPage(): void {
  if (typeof window === 'undefined') return
  if (window.location.pathname.replace(/\/$/, '') !== '/admin/login') return

  // Si ya está autenticado, sincronizar cookie (por si tenía sesión antigua sin cookie) y redirigir
  if (isAuthenticated()) {
    syncAuthSessionCookieIfNeeded()
  ...
  // Callback OAuth: cuando volvemos de Google/GitHub con ?token= & refresh=
